                ...
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: 'HDFC_UAT_BackEnd_Remote_Server', keyFileVariable: 'identity', passphraseVariable: '', usernameVariable: 'centos')]) {
                      ...
                      remote.allowAnyHosts = true
                        ...
                        sshCommand remote: remote, command: 'rsync -avzru --no-perms --no-owner --no-group /var/www/html/smartbuy3.0/angular-smartbuy /home/apache/git/backups/code_backup_AngularUI4.0_`date +%Y%m%d` --exclude="application/logs" --exclude=".git" --exclude="resources/temp" --exclude="uploads"  --exclude="OrderStatusFiles"'
                        echo "Backup Completed"
                        
                        echo "Deploying the AngularUI4.0 Module"
                        sh 'rsync -avzr --no-perms --no-owner /var/lib/jenkins/workspace/HDFC-SMARTBUY-FRONTEND-PIPELINE-AngularUI4.0/ apache@10.80.2.72:/var/www/html/smartbuy3.0/angular-smartbuy/ --exclude=application/logs --exclude=.git --exclude=application/config/database.php --exclude=application/config/mongo_db.php --exclude=.env --exclude=config --exclude=storage'
